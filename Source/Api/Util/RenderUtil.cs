...
public static class RenderUtil
{
    ...
    public static void setGlossiness(Renderer r, float specular, float shininess, float fresnel,
            ...
            for (var i = 0; i < r.materials.Length; i++)
            {
                ...
                if ((textureLayers == null && i == 0) || (textureLayers != null && textureLayers.ContainsKey(i)))
                {
                    ...
                    if (path.EndsWith("/", StringComparison.InvariantCultureIgnoreCase) &&
                        suffix.StartsWith("_", StringComparison.InvariantCultureIgnoreCase))
    ...
    {
        foreach (var rr in r) swapToModdedTextures(rr, pfb);
    }
    ...
    public static void setMesh(GameObject go, Mesh m)
    {
        ...
        var c = m.colors;
        for (var i = 0; i < c.Length; i++) c[i] = new Color(c[i].r, c[i].g, c[i].b, 1);
        m.colors = c;
        foreach (var mf in go.GetComponentsInChildren<MeshFilter>(true)) mf.mesh = m;
    ...
    {
        var renderTex = RenderTexture.GetTemporary(source.width, source.height, 0, RenderTextureFormat.Default,
    ...
    {
        foreach (var m in r.materials) dumpTextures(a, m, r.gameObject.name + "_$_");
    }
    ...
    public static void dumpTexture(Assembly a, string fn, Texture2D img, string pathOverride = null)
    {
        if (img != null)
        {
            var raw = duplicateTexture(img).EncodeToPNG();
