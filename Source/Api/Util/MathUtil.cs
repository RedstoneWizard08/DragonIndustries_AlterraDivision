...
public static class MathUtil
{
    ...
    {
        var x = Random.Range(rec.xMin, rec.xMax);
        var z = Random.Range(rec.yMin, rec.yMax);
        return new Vector2(x, z);
    }

    public static bool isPointInsideEllipse(double x, double y, double z, double ra, double rb, double rc)
    {
        return (ra > 0 ? x * x / (ra * ra) : 0) + (rb > 0 ? y * y / (rb * rb) : 0) + (rc > 0 ? z * z / (rc * rc) : 0) <=
               1;
    }
    ...
    {
        var r = Random.Range(r0, r1);
        var ang = Random.Range(0, 360F);
    ...
    public static double getDistanceToLine(Vector3 point, double x1, double y1, double z1, double x2, double y2,
        double z2)
    {
    ...
    public static double getScalarOfClosestPointToLineSegment(Vector3 point, double x1, double y1, double z1, double x2,
        double y2, double z2)
    {
    ...
    public static Vector3 getClosestPointToLineSegment(Vector3 point, double x1, double y1, double z1, double x2,
        double y2, double z2)
    {
        var t = (float) getScalarOfClosestPointToLineSegment(point, x1, y1, z1, x2, y2, z2);
        return Vector3.Lerp(new Vector3((float) x1, (float) y1, (float) z1),
            new Vector3((float) x2, (float) y2, (float) z2), t);
    }

    public static double getDistanceToLineSegment(Vector3 point, double x1, double y1, double z1, double x2, double y2,
        double z2)
    {
