...
public abstract class CustomMachine<M> : ScanningGadget, DIPrefab<CustomMachine<M>, StringPrefabContainer>
    where M : CustomMachineLogic
{
    ...
    protected CustomMachine(string id, string name, string desc, string template) : base(
        new CustomPrefab(id, name, desc))
    {
        ...
        DIMod.machineList[prefab.Info.TechType] = this;
        SaveSystem.addSaveHandler(prefab.Info.ClassID, saveHandler);
        if (page != null)
            TechnologyUnlockSystem.instance.registerPage(prefab.Info.TechType, page);

        prefab.SetPdaGroupCategory(isOutdoors() ? TechGroup.ExteriorModules : TechGroup.InteriorModules,
    ...
    {
        con.storageRoot.ClassId = prefab.Info.ClassID.ToLowerInvariant() + "container";
        if (string.IsNullOrEmpty(label))
            label = prefab.Info.PrefabFileName;
    ...
    {
        SNUtil.log("Creating " + fragments.Length + " fragments for " + this + " from " + fragments.toDebugString(),
            ownerMod);
        foreach (var m in fragments)
        {
            m.target = prefab.Info.TechType;
            m.fragmentPrefab = GenUtil.getOrCreateFragment(prefab, m.template, m.objectModify);
            SNUtil.log("Registered fragment " + m.fragmentPrefab.Info.ClassID, ownerMod);
        }

        SNUtil.addPDAEntry(fragments[0].fragmentPrefab, scanTime, null, null, null, e =>
        {
            e.blueprint = prefab.Info.TechType;
            e.destroyAfterScan = shouldDeleteFragments();
            e.isFragment = true;
            e.totalFragments = needed;
            e.key = GenUtil.getFragment(prefab.Info.TechType, 0).Info.TechType;
            if (page != null)
    ...
    {
        page = PDAManager.createPage("ency_" + prefab.Info.ClassID, prefab.Info.PrefabFileName, text,
    ...
    public GameObject GetGameObject()
    {
        ...
        ctr.techType = prefab.Info.TechType;
...
public abstract class CustomMachineLogic : MonoBehaviour
{
    ...
    internal ScanningGadget prefab;
    ...
    private void Update()
    {
        ...
        if (needsAttachedBase())
        {
            ...
            if (!par || !par.GetComponent<SubRoot>()) findClosestSub();
            if (!sub)
            {
                sub = gameObject.GetComponentInParent<SubRoot>();
                if (!sub) findClosestSub();
            }
        }
    }
    ...
    private void tryGetPrefab()
    {
        ...
        if (tt != TechType.None && DIMod.machineList.ContainsKey(tt)) prefab = DIMod.machineList[tt];
    }
    ...
    private void findClosestSub()
    {
        ...
        SNUtil.log(
            ...
            " did not have proper parent component hierarchy: " + transform.parent, SNUtil.diDLL);
        foreach (var s in FindObjectsOfType<SubRoot>())
        {
            ...
            if (dist > 350)
                continue;
            if (!sub || dist < Vector3.Distance(sub.transform.position, transform.position)) sub = s;
        }
