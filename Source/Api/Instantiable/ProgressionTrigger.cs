...
public class TechUnlockEffect : DelayedProgressionEffect
{
    ...
    public TechUnlockEffect(TechType tt, float chance = 1, float mind = 0) : base(() => unlockTech(tt),
        () => KnownTech.knownTech.Contains(tt), chance, mind)
    {
        unlock = tt;
    }
...
public class DelayedEncyclopediaEffect : DelayedProgressionEffect
{
    ...
    public DelayedEncyclopediaEffect(PDAManager.PDAPage g, float f, float mind = 0, bool doSound = true) : base(
        () => g.unlock(doSound), g.isUnlocked, f, mind)
    {
        page = g;
        pageKey = g.id;
    }
...
public class DelayedStoryEffect : DelayedProgressionEffect
{
    ...
    public DelayedStoryEffect(StoryGoal g, float f, float mind = 0) : base(() => StoryGoal.Execute(g.key, g.goalType),
        () => StoryGoalManager.main.completedGoals.Contains(g.key), f, mind)
    {
        goal = g;
        goalKey = g.key;
    }
