...
[Serializable]
public sealed class PlacedObject : DIPositionedPrefab
{
    public new static readonly string TAGNAME = "object";
    ...
    [SerializeField] internal int referenceID;

    [SerializeField] public GameObject obj;

    [SerializeField] internal GameObject fx;
    ...
    private void createFX()
    {
        try
        {
            ...
            if (bubblePrefab == null) bubblePrefab = ObjectUtil.lookupPrefab(BUBBLE_PREFAB);
            if (bubblePrefab)
            {
    ...
    public override void replaceObject(string pfb)
    {
    ...
    public void setSelected(bool sel)
    {
        ...
        catch (Exception ex)
        {
            ...
            SNUtil.log("Could not set enabled visual of " + this + " due to FX (" + fx + ") GO error: " + ex,
                SNUtil.diDLL);
        }
    }
    ...
    public override string ToString()
    {
        try
        {
            var t = obj.transform;
            var pos = t == null
                ? "null-transform @ " + position + " / " + rotation + " / " + scale
                : t.position + " / " + t.rotation.eulerAngles + " / " + t.localScale;
            return prefabName + " [" + tech + "] @ " + pos + " (" + referenceID + ")" + " " + (isSelected ? "*" : "");
        }
        catch (Exception ex)
        {
            return "Errored " + prefabName + " @ " + position + ": " + ex;
        }
    }
    ...
    private void nestObject(GameObject go, XmlElement e)
    {
        ...
        if (p != null)
        {
            ...
            p.saveToXML(e2);
            e.AppendChild(e2);
            foreach (Transform t in go.transform) nestObject(t.gameObject, e2);
        }
    }
                    ...
                    if (bc != null)
                    {
                        ...
                        foreach (Transform t2 in t)
                        {
                            ...
                            else
                            {
                                ...
                                p3.saveToXML(e3);
                                e2.AppendChild(e3);
                                foreach (Transform t3 in t2) nestObject(t3.gameObject, e3);
                            }
                        }
    ...
    public override void loadFromXML(XmlElement e)
    {
        ...
        foreach (var mb in manipulations) mb.applyToObject(this);
    }
    ...
    {
        var pfb = new DIPositionedPrefab("");
        pfb.loadFromXML(e);
    ...
    internal static PlacedObject createNewObject(DIPositionedPrefab pfb)
    {
    ...
    internal static PlacedObject createNewObject(GameObject go)
    {
        ...
        if (pi == null && go.name.StartsWith("Base", StringComparison.InvariantCulture))
        {
            ...
            if (id == null)
            {
                ...
                if (tt != null && tt.type == TechType.BaseFoundation) id = "Base_Foundation";
            }
        }
